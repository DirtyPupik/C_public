...
void insert_node_after (node_t *node_to_insert_after, node_t *new_node) {
    ...
    if (new_node->next != NULL) {
        new_node->next->prev = new_node;     // set the next's node prev to the new node
    }
...
void remove_node(node_t **head, node_t *node_to_remove){
    if (*head == node_to_remove) {      // special case - if head node is the one to remove
        *head = node_to_remove->next;
        if (*head != NULL) {
            (*head)->prev = NULL;
        }
    } else {
        node_to_remove->prev->next = node_to_remove->next;      // link the previous node's "next" pointer to the removed node's next node
